...
def get_batch_to_dataloader(get_batch_method_):
    class DL(PriorDataLoader):
        ...
        def __init__(self, num_steps, **get_batch_kwargs):
            ...
            self.num_features = get_batch_kwargs.get(
                'num_features') or self.num_features
            self.epoch_count = 0
        ...
        def gbm(*args, eval_pos_seq_len_sampler, **kwargs):
            ...
            if 'dynamic_batch_size' in kwargs and kwargs['dynamic_batch_size'] > 0 and kwargs['dynamic_batch_size']:
                kwargs['batch_size'] = kwargs['batch_size'] * math.floor(math.pow(
                    kwargs['seq_len_maximum'], kwargs['dynamic_batch_size']) / math.pow(kwargs['seq_len'], kwargs['dynamic_batch_size']))
            batch = get_batch_method_(*args, **kwargs)
            x, y, target_y, style = batch if len(batch) == 4 else (
                batch[0], batch[1], batch[2], None)
        ...
        def __iter__(self):
            assert hasattr(
                self, 'model'), "Please assign model with `dl.model = ...` before training."
            self.epoch_count += 1
...
def plot_features(data, targets, fig=None, categorical=True):
    ...
    for d in range(0, data.shape[1]):
        for d2 in range(0, data.shape[1]):
            ...
            if d == d2:
                if categorical:
                    sns.kdeplot(data[:, d], hue=targets[:],
                                ax=sub_ax, legend=False, palette="deep")
                else:
...
def trunc_norm_sampler_f(mu, sigma): return lambda: stats.truncnorm(
    (0 - mu) / sigma, (1000000 - mu) / sigma, loc=mu, scale=sigma).rvs(1)[0]
...
def scaled_beta_sampler_f(a, b, scale, minimum): return lambda: minimum + \
    round(beta_sampler_f(a, b)() * (scale - minimum))


def order_by_y(x, y):
    order = torch.argsort(y if random.randint(0, 1) else -y, dim=0)[:, 0, 0]
    order = order.reshape(2, -1).transpose(0,
                                           1).reshape(-1)  # .reshape(seq_len)
...
def randomize_classes(x, num_classes):
    ...
    random_classes = torch.randperm(
        num_classes, device=x.device).type(x.type())
...
class CategoricalActivation(nn.Module):
    ...
    def forward(self, x):
        ...
        hid_strength = torch.abs(x).mean(0).unsqueeze(
            0) if self.keep_activation_size else None

        categorical_classes = torch.rand(
            (x.shape[1], x.shape[2])) < self.categorical_p
        ...
        for b in range(x.shape[1]):
            ...
            boundaries_rel = class_boundaries[:, b,
                                              categorical_classes[b]].unsqueeze(1)
            x[:, b, categorical_classes[b]] = (x_rel > boundaries_rel).sum(
                dim=0).float() - num_classes / 2
